...
const combinedCatalogs = [...beverages, ...snacks, ...noodles, ...cannedGoods];
...
- **On Scanner Success**:
  ...
  5. If no match is found, preserve the `sku` value, leave the product name blank, and show a brief toast: _"Barcode scanned. Product details not in database; please type details manually."_
...
- **Continuous Scan Logic**:
  ...
  - **On Scanner Success**:
    ...
    3. **If Found**:
       ...
       - Display a visual on-screen banner at the bottom of the viewfinder overlay indicating _"Scanned: [Product Name] (+1)"_ along with the new cart item count and running total.
    ...
    4. **If Not Found**:
       ...
       - Display a floating toast warning: _"Product not registered in inventory (SKU: X)"_.
