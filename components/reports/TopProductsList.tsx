...
export function TopProductsList({ products, maxValue }: TopProductsListProps) {
	...
	return (
		<View>
			{products.map((product, index) => {
				...
				return (
					<View key={product.id} className={index > 0 ? 'mt-3 pt-3 border-t border-dashed border-ink-200' : ''}>
						<View className="flex-row items-baseline justify-between">
							<View className="flex-row items-baseline flex-1 mr-3">
								...
								<View className="flex-1">
									...
									>
										{product.unitsSold} {product.unitsSold === 1 ? 'unit' : 'units'} moved out
									</StyledText>
								</View>
							</View>
							<MoneyText
