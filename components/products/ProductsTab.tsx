...
export default function ProductsTab({
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      lowStock: relevantProducts.filter((p) => p.quantity < LOW_STOCK_THRESHOLD)
        .length,
    ...
    <View className="flex-1">
      ...
      <View className="px-4 py-3">
        <View className="flex-row items-center">
          <View className="flex-1 bg-white rounded-2xl flex-row items-center px-4 py-3 shadow-sm mr-3">
            <FontAwesome name="search" size={16} color="#A8A29E" />
            <TextInput
              placeholder="Search by name or SKU..."
              value={search}
              onChangeText={setSearch}
              className="flex-1 ml-3 font-stack-sans text-base text-warm-900"
              placeholderTextColor="#A8A29E"
            />
            {search.length > 0 && (
              <Pressable onPress={() => setSearch('')} className="p-1">
                <FontAwesome name="times-circle" size={16} color="#A8A29E" />
              </Pressable>
            )}
          </View>
          <TouchableOpacity
            ...
            hitSlop={20}
            activeOpacity={0.8}
            className="bg-primary-500 rounded-2xl p-6 justify-center items-center"
          >
      ...
      <View className="flex-row justify-between px-4 gap-3 pb-2">
        <View className="flex-1 bg-white rounded-2xl p-3 border border-warm-100">
          ...
          <StyledText variant="extrabold" className="text-primary-500 text-xl">
            {stats.total}
          </StyledText>
        </View>
        <View className="flex-1 bg-white rounded-2xl p-3 border border-warm-100">
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <ProductItem
            item={item}
            onEdit={(id) =>
              router.push(`/(edit-forms)/edit-product/${id}` as any)
            }
