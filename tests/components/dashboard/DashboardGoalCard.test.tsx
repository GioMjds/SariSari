...
import { HomeRecommendation } from '@/components/dashboard/home-state';
...
jest.mock('react-i18next', () => ({
  useTranslation: () => ({
    t: (key: string, options?: any) => {
      ...
      if (typeof key === 'string' && key.includes('continueSelling.title')) return 'Store is active';
      if (typeof key === 'string' && key.includes('continueSelling.cta')) return 'New Sale';
      if (typeof key === 'string' && key.includes('dashboardTitle')) return 'Counter Command Center';
      if (typeof key === 'string' && key.includes('dashboardSubtitle')) return 'Store Assistant Active';
      if (typeof key === 'string' && key.includes('drawerActive')) return 'Drawer Active';
      if (typeof key === 'string' && key.includes('drawerClosed')) return 'Drawer Closed';
