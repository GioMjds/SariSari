...
export default function OnboardingPage() {
  ...
  const isLastTour =
    step.kind === 'tour' && step.tab === TOUR_ORDER[TOUR_ORDER.length - 1];

  const isProfileComplete =
    profile.ownerName.trim().length > 0 && profile.storeName.trim().length > 0;
  ...
  const continueLabel = isProfile
	...
	// TODO: UI redesign of onboarding screen to follow modernistic, professional, and informative UI/UX
  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      ...
      <View className="px-6 pt-4">
        <View className="flex-row justify-between items-center">
          ...
          <OnboardingPagination
            ...
            maxReachableIndex={isProfileComplete ? maxIndex : PROFILE_INDEX}
            onJump={handleJump}
          />
        </View>
      </View>
      ...
      <View className="flex-1 px-6 pt-6 pb-4">
        {isProfile && <ProfileStep profile={profile} onChange={setProfile} />}

        {!!tourStep && <TourCard step={tourStep} stepKey={currentIndex} />}
