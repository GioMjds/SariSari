...
- **Local Notification** – When the configured time arrives, the app posts a local notification (“Time for your weekly stocktake”). Tapping the notification opens the Stocktake screen directly.
...
| Pause / Resume Across Sessions | If the app is backgrounded or the device rebots while a session is in progress, the counting view is restored automatically on relaunch. | No extra UI needed; handled by checking for an `in_progress` session on app start. |
